...
<head>
    ...
    <script type="text/javascript">
        $(function () {
            ...
            var addCartTs = $("[name = 'addCartT']");
            addCartTs.click(function () {
                var pid = $(this).val();
                var count = $(this).parent().prev().children("[name = 'countT']").text();
                $.ajax({
                    type: "post",
                    url: "${pageContext.request.contextPath}/cart/addCart.do",
                    data: {
        ...
        <div class="shop-right-con">
            ...
            <div class="paging">
                <div class="pag-left f-l">
                    ...
                    <a href="${pageContext.request.contextPath}/product/productSearch.do?cid=${productWapper.cid}&csid=${productWapper.csid}&pname=${productWapper.pname}&pageNow=1"
                    ...
                    <ul class="left-m f-l">
                        ...
                        <c:if test="${productByCid.page.preGroupForPage!=-1}">
                            <li>
                                <a href="${pageContext.request.contextPath}/product/productSearch.do?cid=${productWapper.cid}&csid=${productWapper.csid}&pname=${productWapper.pname}&pageNow=${productByCid.page.preGroupForPage}">...</a>
                            </li>
                        </c:if>
                        <c:forEach items="${productByCid.page.pages}" var="pagei">
                            <li class="${pagei==productByCid.pageNow?"current":""}"><a
                                    href="${pageContext.request.contextPath}/product/productSearch.do?cid=${productWapper.cid}&csid=${productWapper.csid}&pname=${productWapper.pname}&pageNow=${pagei}">${pagei}</a>
                            </li>
                        </c:forEach>
                        ...
                        <c:if test="${productByCid.page.nextGroupForPage!=-1}">
                            <li>
                                <a href="${pageContext.request.contextPath}/product/productSearch.do?cid=${productWapper.cid}&csid=${productWapper.csid}&pname=${productWapper.pname}&pageNow=${productByCid.page.nextGroupForPage}">...</a>
                            </li>
                        </c:if>
                        <c:if test="${productByCid.page.nextGroupForPage!=-1}">
                            <li>
                                <a href="${pageContext.request.contextPath}/product/productSearch.do?cid=${productWapper.cid}&csid=${productWapper.csid}&pname=${productWapper.pname}&pageNow=${productByCid.totalPage}">${productByCid.totalPage}</a>
                            </li>
                        </c:if>
                        <div style="clear:both;"></div>
                    </ul>
                    <a href="${pageContext.request.contextPath}/product/productSearch.do?cid=${productWapper.cid}&csid=${productWapper.csid}&pname=${productWapper.pname}&pageNow=${productByCid.totalPage}"
                       class="about left-l f-l">></a>
                    <div style="clear:both;"></div>
                </div>
                <div class="pag-right f-l">
                    <div class="jump-page f-l">
                        到第<input form="${forms}" value="${pageTY}" id="page" name="pageNow" type="number"
                                 max="${productByCid.totalPage}" maxlength="20" min="1"/>页
                        <input form="${forms}" id="pageTY" name="pageTY" type="hidden" max="${productByCid.totalPage}"
                               maxlength="20" min="1"/>
                    </div>
                    <button class="f-l" type="submit" form="${forms}" id="tiaoye">确定</button>
                    <div style="clear:both;"></div>
                </div>
                <div style="clear:both;"></div>
            </div>
